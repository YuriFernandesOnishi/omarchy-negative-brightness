...
Panel {
  ...
  // Which monitor the brightness slider targets. Independent from Hyprland's
  // "focused" monitor concept — the user picks it explicitly via the pills
  // below the slider, so dragging it never lands on the wrong screen.
  ...
  function setBrightness(value) {
    ...
    var targetMonitor = root.brightnessMonitor
    root.brightnessPercent = percent
    root.pendingBrightnessPercent = percent
    root.pendingBrightnessMonitor = targetMonitor
    root.brightnessByMonitor[targetMonitor] = percent
    ...
    setBrightnessProc.command = ["omarchy-brightness-extended", targetMonitor, "set", String(percent)]
    setBrightnessProc.running = true
  }

  // Switch which monitor the slider controls. Shows its last known value
  // immediately if we have one cached; otherwise reads it fresh.
  function selectBrightnessMonitor(name) {
    if (!name || name === root.brightnessMonitor) return
    root.brightnessMonitor = name
    var cached = root.brightnessByMonitor[name]
    if (cached !== undefined) {
      root.brightnessPercent = Model.clampBrightness(cached)
    } else {
      root.refresh()
    }
  }
  ...
  Process {
    id: stateProc
    command: ["omarchy-monitor-state-extended"]
    stdout: StdioCollector {
      ...
      onStreamFinished: {
        ...
        var byMonitor = {}
        try { byMonitor = JSON.parse(String(lines[8] || "{}").trim()) } catch (e) { byMonitor = {} }
        root.brightnessByMonitor = byMonitor

        // Keep the current selection if it still exists; otherwise fall back
        // to the focused monitor, then whatever the CLI reported.
        var stillValid = root.brightnessMonitor && root.displays.some(function(d) { return d.name === root.brightnessMonitor })
        if (!stillValid)
          root.brightnessMonitor = root.focusedMonitor || (root.displays[0] ? root.displays[0].name : "")

        var known = root.brightnessByMonitor[root.brightnessMonitor]
        root.brightnessPercent = root.brightnessAvailable
          ? (known !== undefined ? Model.clampBrightness(known) : Math.max(-70, Math.min(100, parseInt(brightness, 10))))
          : 0
      }
    }
  }
  ...
  Process {
    ...
    onRunningChanged: {
      ...
      if (root.brightnessSetQueued) {
        // Re-dispatch to whichever monitor the queued change was originally
        // for — not root.brightnessMonitor, which may have changed since if
        // the user switched the picker mid-flight.
        root.brightnessSetQueued = false
        setBrightnessProc.command = ["omarchy-brightness-extended", root.pendingBrightnessMonitor, "set", String(root.pendingBrightnessPercent)]
        setBrightnessProc.running = true
      }
    }
  }
      ...
      ScrollView {
        ...
        Column {
          ...
          Column {
            ...
            // Which monitor the slider below targets. Only shown with more
            // than one display — picking one here is independent of Hyprland
            // focus, so the slider never lands on the wrong screen.
            Grid {
              id: brightnessMonitorRow
              visible: root.displays.length > 1
              width: parent.width
              columns: root.displays.length
              spacing: Style.spacing.xs

              readonly property real cellWidth: root.displays.length > 0
              ...
              Repeater {
                ...
                MonitorPill {
                  ...
                  display: modelData
                  width: brightnessMonitorRow.cellWidth
                }
              }
            }
  ...
  component MonitorPill: Button {
    ...
    active: root.brightnessMonitor === display.name

    onClicked: root.selectBrightnessMonitor(display.name)
  }
